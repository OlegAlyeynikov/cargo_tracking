...
_CHAR_VALUES = {
    c: i + (i // 11) for i, c in enumerate("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ")
}
...
def _iso6346_check_digit(number: str) -> bool:
    ...
    if len(number) != 11:
        return False
    total = sum(
        _CHAR_VALUES.get(c, 0) * _ISO6346_WEIGHTS[i] for i, c in enumerate(number[:10])
    )
...
def detect(number: str) -> DetectedInfo:
    ...
    if _AWB_PATTERN.match(normalized):
        canonical = (
            normalized if "-" in normalized else f"{normalized[:3]}-{normalized[3:]}"
        )
        prefix = canonical[:3]
