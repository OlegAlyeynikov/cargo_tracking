...
def test_unknown_status_returns_unknown() -> None:
    assert normalize_status("some completely unknown status XYZ", "air_awb") == "unknown"
    assert normalize_status("some completely unknown status XYZ", "sea_container") == "unknown"
